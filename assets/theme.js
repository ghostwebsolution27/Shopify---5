...
theme.Cart = (function() {
  var selectors = {
    ...
    cartItemHandle: '[data-cart-item-handle]',
    ...
    cartItemEntry: '[data-cart-item-entry]',
    cartItemEntryValue: '[data-cart-item-entry-value]'
  };
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    _setupCartTemplates: function() {
      ...
      this.itemEntryTemplate = this.itemTemplate
        .querySelector(selectors.cartItemEntry)
        .cloneNode(true);
    },
    ...
    _createCart: function(state) {
      ...
      let totalEntry = 0;
      $('.total-entries__count span').text(0);
      $('.total-entries__count span').addClass('hide');
      for (let i = 0; i < state.items.length; i++) {
        jQuery.getJSON('/products/' + state.items[i].handle + '.js', function(product) {
          let multiplier = 0;
        
          if (product.tags.indexOf('always_entry_product') > -1) {
            let tagLength = product.tags.length;
            for (let j = 0; j < tagLength; j++) {
              if (product.tags[j].includes('entry_x_')) {
                let buf = product.tags[j].split('entry_x_');
                multiplier = buf[1];
                break;
              } 
            }
          ...
          if (multiplier == 0) {
            multiplier = theme.settings.productMultiplier;
          }

          var itemNode = $('tr[data-cart-item-key="' + state.items[i].key + '"]');
          let entry = state.items[i].quantity * state.items[i].original_price / 100 * multiplier;
          itemNode.find('.product-entry span').text(entry);
          // itemNode.find('.product-entry').removeClass('hide');

          let buf = $('.total-entries__count span').text().trim();
          totalEntry = (1 * buf) + entry;
          $('.total-entries__count span').text(totalEntry);
        });
      }
      setTimeout(() => {
        $('.product-entry').removeClass('hide');
        $('.total-entries__count span').removeClass('hide');  
      }, 500);
    ...
    _createLineItemList: function(state) {
      return state.items.map(
        function(item, index) {
          ...
          var itemEntryList = this.itemEntryTemplate.cloneNode(true);
          ...
          itemEntryList = this._createItemEntry(
            item.original_price,
            itemNode.getAttribute(attributes.cartItemQuantity)
          );
          itemEntryList.classList.add('hide');
          
          this._setItemEntry(itemNode, itemEntryList);
    ...
    _setLineItemAttributes: function(itemNode, item, index) {
      ...
      itemNode.setAttribute(attributes.cartItemHandle, item.handle);
    },
    ...
    _setItemEntry: function(item, entry) {
      item.querySelector(selectors.cartItemEntry).innerHTML = entry.innerHTML;
    },
    ...
    _createItemEntry: function(original_price, quantity) {
      var resultHTML = this.itemEntryTemplate.cloneNode(true);

      resultHTML.querySelector(
        selectors.cartItemEntryValue
      ).innerHTML = ''; //quantity * original_price / 100;

      resultHTML.classList.add('hide');
      return resultHTML;
    },
...
$(document).ready(function() {
  ...
  $('.js-drawer-open-left').click(function() {
    if ($('#NavDrawer').hasClass('open-navdrawer')) {
      $('#NavDrawer').removeClass('open-navdrawer');
      $('#PageContainer').removeClass('open-navdrawer');
      $('body').removeClass('open-navdrawer');
    } else {
      $('#NavDrawer').addClass('open-navdrawer');
      $('#PageContainer').addClass('open-navdrawer');
      $('body').addClass('open-navdrawer');
    }
  });
  ...
  $('.drawer-nav__has-sublist').click(function() {
    $(this).toggleClass('drawer-nav--expanded');
  });


  /********** quick faq section start *********/
  $('.quick-faq .faq-title').click(function() {
    if ($(this).parent().hasClass('open')) {
      $(this).parent().removeClass('open');
      $(this).next().css('max-height', '0');
    } else {
      $(this).parents('.faq-list').find('.faq.open .faq-content').css('max-height', '0');
      $(this).parents('.faq-list').find('.faq.open').removeClass('open');
      $(this).parent().addClass('open');
      $(this).parent().find('.faq-content').css('max-height', $(this).parent().find('.faq-content').prop('scrollHeight') + 'px');
    }
  });
  /********** quick faq section start *********/
});